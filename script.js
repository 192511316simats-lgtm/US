...
(function () {
  ...
  // Plays romantic acoustic Tamil melody notes & chords if audio file is not present
  function startRomanticSynth() {
    try {
      ...
      // Romantic notes scale (A acoustic minor / C major romantic progression)
      // Un Vizhigalil melodic motif frequencies
      const melodyNotes = [
        329.63, 392.00, 440.00, 493.88, 523.25, 493.88, 440.00, 392.00,
        349.23, 392.00, 440.00, 392.00, 349.23, 329.63, 293.66, 329.63
      ];
      let noteIndex = 0;
      ...
      synthInterval = setInterval(() => {
        ...
        // Warm romantic electric piano / flute harmonic tone
        osc.type = 'sine';
  ...
  function toggleMusic() {
    if (isMusicPlaying) {
      ...
      if (bgAudio) {
        bgAudio.play().catch(() => {
          // If no external mp3 file exists, fallback to Web Audio romantic synth
          startRomanticSynth();
        });
      } else {
        startRomanticSynth();
